...
import { categories, featuredEvents, upcomingEvents } from '#/components/discovery/data'
    ...
    <SitePage>
        ...
        <div className="grid grid-cols-[1.3fr_1fr] gap-3.5">
          ...
          >
            ...
            <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/75 to-transparent p-5 text-white">
              <div className="mb-1.5 text-[12px] font-bold opacity-85">Muziek · Paramaribo</div>
              <div className="text-[19px] font-extrabold">Neon Night Festival</div>
            </div>
          </div>
          <div className="relative min-h-[130px] overflow-hidden rounded-[16px]" style={stripe('#EFF6FF', '#DBEAFE')}>
            <div className="absolute inset-x-2.5 bottom-2.5 text-[12px] font-bold text-[#0F172A]">Creative Workshop</div>
          </div>
      ...
      <section className="mx-auto mb-16 max-w-[1280px] px-6 md:px-12">
        ...
        <div className="flex gap-[18px] overflow-x-auto pb-1.5">
          {categories.map((cat) => (
            <Link to="/events" key={cat.name} className="w-[190px] flex-none cursor-pointer">
        ...
        <div className="mx-auto max-w-[1280px]">
          ...
          <div className="grid gap-6 md:grid-cols-3">
            {featuredEvents.map((ev) => (
              ...
              >
                <div className="relative h-[170px]" style={ev.pattern}>
                ...
                <div className="p-[18px]">
                  ...
                  <div className="mb-1.5 text-[17px] font-extrabold">{ev.title}</div>
                  <div className="mb-4 flex items-center gap-1.5 text-[13px] text-[#64748B]">
                    <PinIcon />
                    {ev.location}
                  </div>
                  <div className="flex items-center justify-between">
                    <div className="text-[15px] font-extrabold text-[#2563EB]">{ev.price}</div>
      ...
      <section className="mx-auto max-w-[1000px] px-6 py-16 md:px-12">
        ...
        <div className="flex flex-col gap-4">
          {upcomingEvents.map((ev) => (
            ...
            >
              <div className="h-[88px] w-full flex-none rounded-[12px] sm:w-[120px]" style={ev.pattern} />
              <div className="flex-1">
                ...
                <div className="mb-1 text-[16px] font-extrabold">{ev.title}</div>
                <div className="flex items-center gap-4 text-[13px] text-[#64748B]">
                  ...
                  <span className="flex items-center gap-1.5">
                    <UsersIcon size={13} />
                    {ev.participants} deelnemers
                  </span>
                </div>
              </div>
              <div className="flex-none text-right">
                <div className="mb-2 text-[15px] font-extrabold text-[#2563EB]">{ev.price}</div>
                <Link
                  to="/events/$eventId"
                  params={{ eventId: ev.id }}
