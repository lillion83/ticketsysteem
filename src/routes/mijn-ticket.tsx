...
import type { MijnTicket } from '#/server/mijnTickets'
...
export const Route = createFileRoute('/mijn-ticket')({
  loader: async () => {
    ...
    if (!user)
      return {
        user: null,
        tickets: [] as Array<MijnTicket>,
        googleEnabled: await googleBeschikbaar(),
      }
...
type EventGroep = {
  ...
  cover: string | null
  tickets: Array<MijnTicket>
}

// Groepeert de (op datum gesorteerde) tickets per event. De volgorde blijft
// behouden doordat de Map de invoegvolgorde aanhoudt.
function groepeerPerEvent(tickets: Array<MijnTicket>): Array<EventGroep> {
  const groepen = new Map<string, EventGroep>()
  for (const t of tickets) {
    let g = groepen.get(t.event_id)
    if (!g) {
      g = {
        event_id: t.event_id,
        event_naam: t.event_naam,
...
// Korte telling per event, bv. "3 tickets · 1 gebruikt".
function ticketTelling(tickets: Array<MijnTicket>): string {
  const totaal = tickets.length
  const gebruikt = tickets.filter((t) => t.status === 'gebruikt').length
  const basis = `${totaal} ${totaal === 1 ? 'ticket' : 'tickets'}`
  return gebruikt > 0 ? `${basis} · ${gebruikt} gebruikt` : basis
}
  ...
  return (
    <SitePage>
      ...
      <div className="mx-auto max-w-[640px] px-6 py-16">
        ...
        {user ? (
          <Overzicht email={user.email} tickets={tickets} />
        ) : (
          <Inloggen googleEnabled={googleEnabled} />
        )}
        <CodeOpzoeker />
      </div>
      <SiteFooter />
    ...
    <section className="mb-10">
      ...
      {tickets.length === 0 ? (
        <div className="rounded-[16px] border border-dashed border-[#E5E7EB] py-12 text-center text-[14px] text-[#64748B]">
          Nog geen tickets op dit account. Tickets verschijnen hier zodra de
          organisator ze op dit e-mailadres uitgeeft.
        </div>
      ) : (
        <div className="flex flex-col gap-3">
          {groepeerPerEvent(tickets).map((g) => (
            <Link
              key={g.event_id}
              to="/mijn-ticket/$eventId"
              params={{ eventId: g.event_id }}
              className="flex items-center gap-4 rounded-[14px] border border-[#E5E7EB] p-3 transition hover:shadow-md"
            >
              <div
                className="h-[56px] w-[56px] shrink-0 rounded-[10px]"
                style={coverStyle(g.categorie, g.cover)}
              />
              <div className="min-w-0 flex-1">
                ...
                <div className="truncate text-[12.5px] text-[#64748B]">
                  {formatDateLine(new Date(g.datum_start))}
                  {g.locatie ? ` · ${g.locatie}` : ''}
                </div>
                <div className="mt-0.5 text-[12.5px] font-semibold text-[#2563EB]">
                  {ticketTelling(g.tickets)}
                </div>
              </div>
              <svg
                width="18"
  ...
  return (
    <section className="mb-10">
      <p className="mb-6 text-[15px] text-[#64748B]">
        Log in om je tickets met status te bekijken. Tickets staan op het
        e-mailadres waarmee je bij de organisator hebt gekocht of gereserveerd.
      </p>
  ...
  return (
    <section className="border-t border-[#E5E7EB] pt-8">
      <h2 className="mb-2 text-[16px] font-extrabold">
        Heb je alleen een link of code?
      </h2>
      <p className="mb-4 text-[13.5px] text-[#64748B]">
        Plak je ticketlink of -code uit de mail of WhatsApp om je ticket direct
        te openen.
      </p>
      ...
      {fout && (
        <p className="mt-2 text-[13px] font-semibold text-[#EF4444]">{fout}</p>
      )}
    </section>
  )
}
